...
const EditProfile = ({ userId = 0, setVisible }) => {
  ...
  useEffect(() => {
    // si el usuario es administrador va a poder realizar peticiones a la base de datos preguntando por informacion de otros usuarios a modificar
        ...
        {({ errors }) => (
         <div className=" auth__content content-modal">
            ...
            <div className="data-box">
              <label
                className="label"
                htmlFor={actualUser === 1 ? "roleId" : "email"}
              >
                {actualUser === 1 ? "Permisos de usuario" : "Email"}
              </label>
              {actualUser === 1 ? (
                <Field
                  className="input"
              ...
              <ErrorMessage
                name="email"
                component={() => <small className="err">{errors.email}</small>}
              />
            </div>
